...
def select_vid_file():
    ...
    filename = tkinter.filedialog.askopenfilename(initialdir = curr_dir, title = "Select Video File", filetypes = [("Video", ".mp4 .mkv")])
...
def select_sub_file():
    ...
    filename = tkinter.filedialog.askopenfilename(initialdir = curr_dir, title = "Select Subtitle File", filetypes = [("Subtitle", ".srt")])
...
def retrieve_link(bash_command):
    ...
    #tkinter.messagebox.showinfo('Success', 'Please wait while audio is being extracted and sent to server!')
    ...
    tkinter.messagebox.showinfo('Success', 'Room Creation Successful! Share the link or scan the QR to join!')
...
def on_closing():
    ...
    if messagebox.askokcancel("Quit", "Closing this window will stop this session. Are you sure you want to quit?"):
        root.destroy()
...
if __name__ == '__main__':
    ...
    curr_dir = subprocess.run('pwd', capture_output = True, text = True).stdout.strip() + '/' + curr_dir
    ...
    check_sub = tkinter.Checkbutton(root, text = "Add subtitles:", command = change_sub_state, variable = allow_sub, onvalue = 1, offvalue = 0)
    check_sub.deselect()
    ...
    global sub_filename
    sub_filename = ''
    sub_btn = tkinter.Button(root, text = 'Choose File', command = select_sub_file, state=tkinter.DISABLED)
    ...
    radio_server_local = tkinter.Radiobutton(root, text = "Local", variable = server, value = 1)
    ...
    radio_quality_low = tkinter.Radiobutton(root, text = "Low", variable = audio_quality, value = 0)
    radio_quality_medium = tkinter.Radiobutton(root, text = "Medium", variable = audio_quality, value = 1)
    radio_quality_high = tkinter.Radiobutton(root, text = "High", variable = audio_quality, value = 2)
    
    tkinter.Label(root, text = 'Audio Quality: ').grid(row = 4, column = 0, columnspan = 2)
    ...
    check_control = tkinter.Checkbutton(root, text = "Only host can control", variable = host_control, onvalue = 1, offvalue = 0)
    ...
    check_qr = tkinter.Checkbutton(root, text = "Show QR", variable = show_qr, onvalue = 1, offvalue = 0)
    check_qr.select()
